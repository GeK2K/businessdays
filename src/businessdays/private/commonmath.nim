...
commonmath
==========
Some basic math concepts are covered in this module.
]##
...
proc  intersection[T](oa: openArray[T]; lowerBound, upperBound: T;
                      cmpT: (T, T) -> int): seq[T] =
  #[
  ...
  **Assertions:**
  ...
  # limit cases:
  # 1. max(oa) < lowerBound  =>  result = empty sequence
  # 2. max(oa) = lowerBound  =>  result = max(oa)
  # 3. upperBound < min(oa)  =>  result = empty sequence
  # 4. upperBound = min(oa)  =>  result = min(oa)
  # 5. upperBound = lowerBound  =>  result = oa.binarysearch(lowerBound)
  if oa[^1].cmp(lowerBound) < 0:  return @[]
  if oa[^1].cmp(lowerBound) == 0:  return @[lowerBound]
  if upperBound.cmp(oa[0]) < 0:  return @[]
  if upperBound.cmp(oa[0]) == 0:  return @[upperBound]
  ...
  # limit case No 6:
  # oa[i] < lowerBound < upperBound < oa[i+1]  (=> oa[lowerIdx] = oa[i+1])
  #   =>  result = empty sequence
  if oa[lowerIdx].cmp(upperBound) > 0:  return @[]
  # all other cases
  return oa[lowerIdx..upperIdx-1].toSeq


proc  intersection*[T](oa: openArray[T]; lowerBound, upperBound: T;
                       interval: BoundedRealInterval; cmp: (T, T) -> int;
                       isSortedUnique = true): seq[T] =
  ##[
